...
module.exports = (function(){
  ...
  var convert_to_bits = function(buffer){
    ...
    for(i = 0; i < buffer.length; i++) {
      var x = parseInt(buffer[i]).toString(2);
      output += left_pad(x, "0", 8);
  ...
  var parse = function(packet){
    ...
    var mV = extract_mV_from_packet(extract_ch1_from_packet(bits));
    var mA = extract_mA_from_packet(extract_ch1_from_packet(bits));
